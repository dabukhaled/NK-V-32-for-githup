...
window.addEventListener('load', () => {
    ...
    // تهيئة واجهة إدارة قاعدة البيانات
    setTimeout(() => {
        if (typeof initializeDatabaseUI === 'function') {
            try {
                initializeDatabaseUI();
            } catch (error) {
                console.error('خطأ في تهيئة واجهة قاعدة البيانات:', error);
            }
        }
    }, 2000);
});
...
// تهيئة localForage لتخزين البيانات (كمصدر احتياطي)
localforage.config({
    name: 'MoviesApp',
    version: 1.0,
...
// تحميل البيانات من قاعدة البيانات JSON
async function loadAppData() {
    try {
        // التحقق من وجود نظام قاعدة البيانات الجديد
        if (typeof DatabaseManager !== 'undefined') {
            console.log('تحميل البيانات من قاعدة البيانات JSON...');
            const dbData = await DatabaseManager.loadAppData();
            
            if (DatabaseManager.isValidDatabase(dbData)) {
                // تحميل البيانات من قاعدة البيانات
                appState.movies = dbData.movies || [];
                appState.series = dbData.series || [];
                appState.categories = dbData.categories || JSON.parse(JSON.stringify(DEFAULT_CATEGORIES));
                
                // تحميل الإعدادات
                if (dbData.settings) {
                    appState.password = dbData.settings.password || '5555';
                    appState.showSpecialSections = dbData.settings.showSpecialSections || false;
                    appState.viewMode = dbData.settings.viewMode || 'grid';
                    appState.sortBy = dbData.settings.sortBy || 'name';
                    appState.selectedSite = dbData.settings.selectedSite || '';
                    appState.selectedStar = dbData.settings.selectedStar || '';
                    appState.itemsPerPage = dbData.settings.itemsPerPage || 50;
                    appState.openMoviesExternally = dbData.settings.openMoviesExternally || false;
                    appState.zoomLevel = dbData.settings.zoomLevel || 1;
                    appState.currentPageTitle = dbData.settings.currentPageTitle || 'New Koktil-aflam v32';
                }
                ...
                if (movieSitesCat) {
                    if (!movieSitesCat.shortcuts) movieSitesCat.shortcuts = [];
                    if (!movieSitesCat.isSites) movieSitesCat.isSites = true;
                    if (!movieSitesCat.folders) movieSitesCat.folders = [];
                } else {
                ...
                console.log('تم تحميل بيانات التطبيق بنجاح من قاعدة البيانات JSON');
                return;
            } else {
                console.log('ملف قاعدة البيانات غير صالح، سيتم استخدام البيانات الافتراضية');
            }
        }
        
        // إذا فشل تحميل قاعدة البيانات أو غير متوفرة، استخدم localForage
        console.log('تحميل البيانات من التخزين المحلي (localForage)...');
        ...
        if (settings) {
            ...
            appState.zoomLevel = settings.zoomLevel || 1;
        }
        ...
        console.log('تم تحميل بيانات التطبيق بنجاح من التخزين المحلي');
    } catch (error) {
...
async function saveAppData() {
    try {
        // التحقق من وجود نظام قاعدة البيانات الجديد
        if (typeof DatabaseManager !== 'undefined') {
            console.log('حفظ البيانات في قاعدة البيانات JSON...');
            ...
            const saved = await DatabaseManager.saveAppData(dbData);
            
            if (saved) {
                console.log('تم حفظ بيانات التطبيق بنجاح في قاعدة البيانات JSON');
                return;
            } else {
                console.log('فشل حفظ البيانات في قاعدة البيانات JSON، سيتم استخدام localForage');
            }
        }
        
        // إذا فشل حفظ قاعدة البيانات أو غير متوفرة، استخدم localForage
        console.log('حفظ البيانات في التخزين المحلي (localForage)...');
        ...
        console.log('تم حفظ بيانات التطبيق بنجاح في التخزين المحلي');
    } catch (error) {
